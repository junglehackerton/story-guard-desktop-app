...
function DemoIntro({onEnter}:{onEnter:()=>void}) {
  ...
  return <main className={`demo-intro-page ${revealed?'intro-ready':''}`}>
    <header className="demo-intro-nav"><a href="#intro-top" className="demo-intro-brand" aria-label="Story Guard 홈"><img className="story-guard-mark" src={storyGuardMark} alt="" aria-hidden="true" /><span>STORY GUARD</span></a><nav><a href="#intro-case">사례</a><a href="#intro-how">사용 방법</a><a href="#intro-principle">원칙</a><button onClick={onEnter}>핵심 기능 체험하기 <span>→</span></button></nav></header>
