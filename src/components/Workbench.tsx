...
export function WorkbenchNav({page,onPage,project,projects,onProject,demo=false,onReset}: {demo?:boolean;onReset?:()=>void;page:Page;onPage:(page:Page)=>void;project:Project|null;projects:Project[];onProject:(p:Project)=>void}) {
 ...
 return <aside className={`workbench-nav${collapsed?' is-collapsed':''}`}><div className="workbench-nav-top"><button className="wordmark" aria-label="STORY GUARD" onClick={()=>onPage('welcome')}><img src={storyGuardMark} alt="" aria-hidden="true"/><span>STORY GUARD</span></button><button type="button" className="sidebar-toggle" aria-expanded={!collapsed} aria-label={collapsed?'사이드바 펼치기':'사이드바 접기'} title={collapsed?'사이드바 펼치기':'사이드바 접기'} onClick={()=>setCollapsed(value=>!value)}>{collapsed?<PanelLeftOpen size={19}/>:<PanelLeftClose size={19}/>}</button></div>
 ...
 <nav aria-label="작품 메뉴">{MENU.map(([key,Icon])=><button key={key} aria-label={PAGES[key][0]} title={PAGES[key][0]} aria-current={page===key?'page':undefined} onClick={()=>onPage(key)}><Icon size={21}/><span className="nav-label">{PAGES[key][0]}</span></button>)}</nav>
